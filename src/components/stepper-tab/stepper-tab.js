...
export const LinearStepper = ({
  ...
  const renderStep = () => {
    if (activeStep === 0) return renderComponentTab1 && renderComponentTab1();
    // if (activeStep === 3) return renderComponentTab4 && renderComponentTab4();
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      {activeStep === steps.length ? (
        ...
        <React.Fragment>
          <Box sx={{ mt: 2, mb: 1 }}>{renderStep()}</Box>
        </React.Fragment>
      )}
    </Box>
  );
