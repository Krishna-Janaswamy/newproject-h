...
export const AddFlashDeal = (props) => {
  const {
    profiledata,
    onSubmitCallBack,
    propertlistdata,
  ...
  const [addPatryDetails, setAddPatryDetails] = useState([]);
  ...
  const [addFlashDealDetails, setAddFlashDealDetails] = useState([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (resValues.flash_deal_video_file.length !== 0) {
        setErrorVideo(false);
      } else {
        setErrorVideo(true);
      }
      ...
      if (
        ...
        resValues?.flash_deal_video_file.length !== 0
      ) {
        setFormTouch(false);
        setVideoTouch(false);
        setImageTouch(false);
  ...
  const getUpdatedCheckTerms = (data) => {
    const isSelectedValues = data.filter((item) => item?.isSelected);
    if (isSelectedValues.length === 0) {
  ...
  useEffect(() => {
    if (JSON.stringify(postImages?.flash_deal_video_file) !== JSON.stringify(videoFile)) {
      setVideoTouch(true);
    } else {
      setVideoTouch(false);
    }
  }, [postImages?.flash_deal_video_file, videoFile]);
                ...
                <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                  ...
                  <Grid item md={12} xs={6}>
                    <CheckBoxCustom
                      ...
                      handleName={getUpdatedCheckTerms}
                    />
                    {termsError && (
                      <label>
                        <Typography
