...
export const DocumentsUpdateProperty = (props) => {
  const {
    filerepocontent,
    postfilerepohandle,
    propertyPostImages,
...
otherDoc: '',
  ...
  const [initialDoc, setInitialDoc] = useState(filerepocontent);
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(filerepocontent) !== JSON.stringify(initialDoc)) {
    ...
    <>
              ...
              <Grid item md={4} xs={12} ml={4}>
                ...
                {otherError && (
                  <Typography
                    ...
                    variant="body1"
                  >
                    Please select your property other Documents
                  </Typography>
                )}
                {typeError?.otherDoc && (
      ...
      {cancleConfirmationPop && (
        <CustomizedDialogs
          ...
          confirmtext={"Yes"}
          canceltext={"No"}
          handleconfirmation={() => handlepropertyiddelete(propertypostresult)}
        />
      )}
        {cancleConfirmation && (
        <CustomizedDialogs
